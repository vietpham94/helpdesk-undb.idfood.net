                ...
                <?php foreach ($enterprise_directory_items as $item) { ?>
                    <?php if (!empty($item[$role->post_title])) : ?>
                        ...
                        <div class="col-12 col-md-8 col-lg-9">
                            ...
                            <p class="address"><?= __('Địa chỉ: ') . get_field('address', $item[$role->post_title]) ?></p>
                            <p class="phone">
                                <span class="enterprise-phone"><?= __('Điện thoại: ') . get_field('enterprise_phone', $item[$role->post_title]) ?></span>
                                <span class="enterprise-hotline"><?= __('Đường dây nóng: ') . get_field('enterprise_hotline', $item[$role->post_title]) ?></span>
                            </p>
                            <p class="online-contact">
                                <span class="enterprise-phone"><?= __('Email: ') . get_field('enterprise_email', $item[$role->post_title]) ?></span>
                                <span class="enterprise-hotline"><?= __('Website: ') . get_field('enterprise_website', $item[$role->post_title]) ?></span>
                            </p>
                        </div>
                    <?php endif; ?>
                <?php } ?>
